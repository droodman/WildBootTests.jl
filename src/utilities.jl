...
function coldot!(dest::AbstractMatrix, A::AbstractMatrix, B::AbstractMatrix)  # colsum(A .* B)
  s2 = axes(A,2)
  @inbounds @fastmath for i ∈ s2
	  dest[i] = A[1,i] * B[1,i]
  end
  if size(A,1)>1
	  @inbounds @fastmath for i ∈ s2, j ∈ 2:size(A,1)
	    dest[i] += A[j,i] * B[j,i]
	  end
  end
end
...
function coldot!(dest::AbstractArray, A::AbstractArray, B::AbstractArray)
  ...
  @inbounds @fastmath for j ∈ eachindexJ
	  dest[1,J[j]] = A[1,J[j]] * B[1,J[j]]
  end
  if size(A,1) > 1
	  @inbounds @fastmath for j ∈ eachindexJ, i ∈ 2:size(A,1)
...
function coldotplus!(dest::AbstractMatrix, A::AbstractMatrix, B::AbstractMatrix)  # colsum(A .* B)
  @inbounds @fastmath for i ∈ axes(A,2), j ∈ axes(A,1)
	  dest[i] += A[j,i] * B[j,i]
  end
end
...
function colquadform(Q::AbstractMatrix, A::AbstractMatrix) :: AbstractVector
  ...
  @inbounds @fastmath for i ∈ axes(A,2), j ∈ axes(A,1), k ∈ axes(A,1)
...
function matmulminus!(A::Matrix, B::Matrix, C::AbstractMatrix)  # add B*C to A in place
	@inbounds @fastmath for i ∈ eachindex(axes(A,1),axes(B,1)), k ∈ eachindex(axes(A,2), axes(C,2)), j ∈ eachindex(axes(B,2),axes(C,1))
		A[i,k] -= B[i,j] * C[j,k]
	end
end

function matmulplus!(A::Matrix, B::Vector, C::Matrix)  # add B*C to A in place
	@inbounds @fastmath for k ∈ eachindex(axes(A,2), axes(C,2)), i ∈ eachindex(axes(A,1),axes(B,1))
		A[i,k] += B[i] * C[k]
	end
end
...
function matmulplus!(A::Vector, B::Matrix, C::Vector)  # add B*C to A in place
	@inbounds @fastmath for j ∈ eachindex(axes(B,2),axes(C,1)), i ∈ eachindex(axes(A,1),axes(B,1))
		A[i] += B[i,j] * C[j]
	end
end
