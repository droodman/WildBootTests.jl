...
mutable struct StrEstimator{T<:AbstractFloat}
  ...
  Yendog::Matrix{Bool}
  invZperpZperpZperpX::Matrix{T}; XZ::Matrix{T}; YPXY::Matrix{T}; R₁invR₁R₁::Matrix{T}; R₁invR₁R₁X::DesignerMatrix{T}; R₁invR₁R₁Y::DesignerMatrix{T}
...
mutable struct StrBootTest{T<:AbstractFloat}
  ...
  B::Int64; const granularOverride::Union{Missing,Bool}; const auxtwtype::Symbol; const rng::AbstractRNG; maxmatsize::Float16
  ...
  seed::UInt64
	...
	StrBootTest{T}(R, r, R₁, r₁, y₁, X₁, Y₂, X₂, wt, fweights, liml, 
	               fuller, κ, arubin, B, granularOverride, auxtwtype, rng, maxmatsize, ptype, null, jk, scorebs, bootstrapt, clustid, NBootClustVar, NErrClustVar, issorted, robust, small, clusteradj, clustermin,
		...
		begin
			...
			new(R, r, R₁, r₁, 
			    y₁, X₁, Y₂, X₂, 
					...
					B, granularOverride, auxtwtype, rng, maxmatsize, 
					...
					0, nrows(ml ? sc : y₁), ncols(clustid), ncols(X₁), kX₂, ncols(Y₂), WREnonARubin, WREnonARubin ? boottestWRE! : boottestOLSARubin!,
					v)
		end
end
